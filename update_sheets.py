...
clietn_name = os.environ.get("CLIENT_NAME", "test_client")
...
full_update_data = [clietn_name, project_name, datetime.now().strftime("%d_%m_%Y-%H_%M_%S")] + list(data_reports.values()) + list(data_validation.values())

target_cell = sheet.find(project_name, in_column=2)
print(target_cell)
